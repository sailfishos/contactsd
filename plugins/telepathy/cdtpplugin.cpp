...
CDTpPlugin::CDTpPlugin()
    : mController(new CDTpController(this, false)),
...
void CDTpPlugin::init()
{
    ...
    mAccountServiceMapper.initialize();
}
...
void CDTpPlugin::onAccountCreated(const QString & path)
{
    ...
    CDTpPendingRosters *request = mController->requestRosters(account);
    mRosters.append(request);

    connect(request,
            SIGNAL(finished(CDTpPendingRosters *)),
            SLOT(onFinished(CDTpPendingRosters *)));
...
void CDTpPlugin::onAccountReady(Tp::PendingOperation *op)
{
    ...
    connect(tpaccount,
            SIGNAL(accountChanged(CDTpAccount*, CDTpAccount::Changes)),
            SLOT(onAccountChanged(CDTpAccount*, CDTpAccount::Changes)));
    mAccounts.append(tpaccount);
    saveSelfContact(account);

    connect(account.data(), SIGNAL(removed()), SLOT(onAccountRemoved()));
    connect(account.data(), SIGNAL(onlinenessChanged(bool)), SLOT(onOnlinenessChanged(bool)));
    connect(account.data(), SIGNAL(haveConnectionChanged(bool)), SLOT(onConnectionChanged(bool)));
}
...
void CDTpPlugin::onAccountRemoved()
{
    if (sender()) {
        Tp::Account *account = qobject_cast<Tp::Account *>(sender());
        if (account) {
            qDebug() << Q_FUNC_INFO << "Account Removed:" <<
                account->objectPath();
            mStore->deleteContacts(account->objectPath());
        }
    }
...
void CDTpPlugin::onFinished(CDTpPendingRosters *op)
{
    ...
    if (op->isError()) {
        emit error("libtelepathycollectorplugin", op->errorName(),
                op->errorMessage());
    }
    ...
    connect(roster,
            SIGNAL(contactsAdded(QList<QSharedPointer<CDTpContact> >)),
            SLOT(onContactsAdded(QList<QSharedPointer<CDTpContact> >)));
    connect(roster,
            SIGNAL(contactsRemoved(QList<QSharedPointer<CDTpContact> >)),
            SLOT(onContactsRemoved(QList<QSharedPointer<CDTpContact> >)));
}
...
void CDTpPlugin::onContactsChanged(const Tp::Contacts &contactsAdded,
        const Tp::Contacts &contactsRemoved)
{
    foreach (const Tp::ContactPtr &contact, contactsAdded) {
        QSharedPointer<CDTpContact> tpcontact =
            QSharedPointer<CDTpContact>(new CDTpContact(contact), &QObject::deleteLater);
        mStore->sinkToStorage(tpcontact);
    }
}

void CDTpPlugin::onAccountChanged(CDTpAccount *account,
        CDTpAccount::Changes changes)
{
    ...
    if (changes == CDTpAccount::Presence && presence.status != "offline") {
        ...
        CDTpPendingRosters* request = mController->requestRosters(
                account->account());
        mRosters.append(request);
        connect(request,
                SIGNAL(finished(Tp::PendingOperation *)),
                SLOT(onFinished(Tp::PendingOperation *)));
...
{
    qDebug() << Q_FUNC_INFO << ": Saving self contact to Tracker. Account is: " <<
        account->objectPath();
    saveIMAccount(account, CDTpAccount::All);
}

void CDTpPlugin::saveIMAccount(Tp::AccountPtr account,
        CDTpAccount::Changes changes)
{
    qDebug() << Q_FUNC_INFO << ": saving own IM account: " <<
        account->objectPath() << "with changed: " << changes;
...
{
    Live<nco::IMAccount> liveAccount = ::tracker()->liveNode(
            QUrl("telepathy:" + account->objectPath()));
    QString displayName = mAccountServiceMapper.serviceForAccountPath(
            account->objectPath());

    if (displayName.isEmpty()) {
        liveAccount->setImDisplayName(account->displayName());
    } else {
        liveAccount->setImDisplayName(mAccountServiceMapper.serviceForAccountPath(account->objectPath()));
    }
    ...
    Live<nco::PresenceStatus> cstatus = ::tracker()->liveNode(
            CDTpTrackerSink::toTrackerStatus(presence.status));
    ...
    // save avatar
    QString filename;
    bool ok = saveAvatar(account->avatar().avatarData,
            account->avatar().MIMEType,
            QDir::homePath() + "/.contacts/avatars/", filename);
    if (ok) {
...
bool CDTpPlugin::saveAvatar(const QByteArray &data, const QString &mime,
        const QString &path, QString &fileName)
{
    ...
    QString file = path + QString(QCryptographicHash::hash(data,
