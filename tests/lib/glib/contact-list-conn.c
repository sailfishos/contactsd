/*
 * contact-list-conn.c - an example connection
 *
...
G_DEFINE_TYPE_WITH_CODE (TestContactListConnection,
    test_contact_list_connection,
...
struct _TestContactListConnectionPrivate
{
  gchar *account;
...
static void
test_contact_list_connection_init (TestContactListConnection *self)
{
  self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
      TEST_TYPE_CONTACT_LIST_CONNECTION,
      TestContactListConnectionPrivate);
}
              ...
              GParamSpec *spec)
{
  TestContactListConnection *self =
    TEST_CONTACT_LIST_CONNECTION (object);
...
finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (test_contact_list_connection_parent_class)->finalize (
      object);
}
...
{
  TestContactListConnection *self = TEST_CONTACT_LIST_CONNECTION (conn);

  return g_strdup_printf ("%s@%p", self->priv->account, self);
}

gchar *
test_contact_list_normalize_contact (TpHandleRepoIface *repo,
                                     const gchar *id,
                                     gpointer context,
                                     GError **error)
{
...
static gchar *
test_contact_list_normalize_group (TpHandleRepoIface *repo,
                                      const gchar *id,
                                      gpointer context,
                                      GError **error)
{
                     ...
                     TpHandleRepoIface *repos[NUM_TP_HANDLE_TYPES])
{
  repos[TP_HANDLE_TYPE_CONTACT] = tp_dynamic_handle_repo_new
      (TP_HANDLE_TYPE_CONTACT, test_contact_list_normalize_contact, NULL);
  ...
  repos[TP_HANDLE_TYPE_GROUP] = tp_dynamic_handle_repo_new
      (TP_HANDLE_TYPE_GROUP, test_contact_list_normalize_group, NULL);
}
                  ...
                  TestContactListConnection *self)
{
  ...
  g_value_set_string (pair->values + 1,
      test_contact_list_manager_get_alias (manager, contact));
...
avatar_updated_cb (TestContactListManager *manager,
                   ...
                   TestContactListConnection *self)
{
  tp_svc_connection_interface_avatars_emit_avatar_updated (self, contact,
      test_contact_list_manager_get_avatar (manager, contact)->token);
}
                     ...
                     TestContactListConnection *self)
{
  ...
  status = tp_presence_status_new (
      test_contact_list_manager_get_presence (manager, contact),
      NULL);
...
{
  TestContactListConnection *self =
    TEST_CONTACT_LIST_CONNECTION (conn);
  GPtrArray *ret = g_ptr_array_sized_new (1);

  self->priv->list_manager =
    TEST_CONTACT_LIST_MANAGER (g_object_new (
          TEST_TYPE_CONTACT_LIST_MANAGER,
          "connection", conn,
  ...
  for (i = 0; i < contacts->len; i++)
    {
      ...
      tp_contacts_mixin_set_contact_attribute (attributes, contact,
          ...
          tp_g_value_slice_new_string (
            test_contact_list_manager_get_alias (self->priv->list_manager,
              contact)));
    }
}
...
{
  guint i;
  TestContactListConnection *self = TEST_CONTACT_LIST_CONNECTION (object);
  ...
  for (i = 0; i < contacts->len; i++)
    {
      ...
      tp_contacts_mixin_set_contact_attribute (attributes, contact,
          TP_TOKEN_CONNECTION_INTERFACE_ALIASING_ALIAS,
          tp_g_value_slice_new_string (
            test_contact_list_manager_get_avatar (self->priv->list_manager,
              contact)->token));
    }
}
...
constructed (GObject *object)
{
  ...
  void (*chain_up) (GObject *) =
    G_OBJECT_CLASS (test_contact_list_connection_parent_class)->constructed;

  if (chain_up != NULL)
    chain_up (object);

  tp_contacts_mixin_init (object,
      G_STRUCT_OFFSET (TestContactListConnection, contacts_mixin));
  ...
  tp_presence_mixin_init (object,
      G_STRUCT_OFFSET (TestContactListConnection, presence_mixin));
                      ...
                      GError **error)
{
  ...
  for (i = 0; i < contacts->len; i++)
    {
      TpHandle contact = g_array_index (contacts, guint, i);
      TestContactListPresence presence;
      ...
      if (contact == base->self_handle)
        {
          presence = (self->priv->away ? TEST_CONTACT_LIST_PRESENCE_AWAY
              : TEST_CONTACT_LIST_PRESENCE_AVAILABLE);
        }
      else
        {
          presence = test_contact_list_manager_get_presence (
                ...
                GError **error)
{
  ...
  if (status->index == TEST_CONTACT_LIST_PRESENCE_AWAY)
    {
      if (self->priv->away)
        return TRUE;
...
static void
test_contact_list_connection_class_init (
    TestContactListConnectionClass *klass)
{
  ...
  g_type_class_add_private (klass,
      sizeof (TestContactListConnectionPrivate));
  ...
  tp_contacts_mixin_class_init (object_class,
      G_STRUCT_OFFSET (TestContactListConnectionClass, contacts_mixin));
  tp_presence_mixin_class_init (object_class,
      G_STRUCT_OFFSET (TestContactListConnectionClass, presence_mixin),
      status_available, get_contact_statuses, set_own_status,
      test_contact_list_presence_statuses ());
  tp_presence_mixin_simple_presence_init_dbus_properties (object_class);

  klass->properties_class.interfaces = prop_interfaces;
  tp_dbus_properties_mixin_class_init (object_class,
      G_STRUCT_OFFSET (TestContactListConnectionClass, properties_class));
}
...
{
  TestContactListConnection *self =
    TEST_CONTACT_LIST_CONNECTION (aliasing);
  TpBaseConnection *base = TP_BASE_CONNECTION (aliasing);
  ...
  for (i = 0; i < contacts->len; i++)
    {
      ...
      const gchar *alias = test_contact_list_manager_get_alias (
          self->priv->list_manager, contact);
             ...
             DBusGMethodInvocation *context)
{
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      test_contact_list_manager_set_alias (self->priv->list_manager,
...
static void
get_avatar_tokens (TpSvcConnectionInterfaceAvatars *avatars,
                   const GArray *contacts,
                   DBusGMethodInvocation *context)
{
  TestContactListConnection *self = TEST_CONTACT_LIST_CONNECTION (avatars);
...
static void
get_known_avatar_tokens (TpSvcConnectionInterfaceAvatars *avatars,
                            const GArray *contacts,
                            DBusGMethodInvocation *context)
{
  TestContactListConnection *self = TEST_CONTACT_LIST_CONNECTION (avatars);
...
static void
request_avatars (TpSvcConnectionInterfaceAvatars *avatars,
    const GArray *contacts,
    DBusGMethodInvocation *context)
{
  TestContactListConnection *self = TEST_CONTACT_LIST_CONNECTION (avatars);
...
test_contact_list_connection_get_contact_list_manager (TestContactListConnection *self)
{
