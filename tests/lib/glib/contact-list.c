...
/* Abstract base class */
G_DEFINE_TYPE_WITH_CODE (TestContactListBase, test_contact_list_base,
    G_TYPE_OBJECT,
...
G_DEFINE_TYPE_WITH_CODE (TestContactList, test_contact_list,
    TEST_TYPE_CONTACT_LIST_BASE,
    G_IMPLEMENT_INTERFACE (TP_TYPE_SVC_CHANNEL, list_channel_iface_init))

/* Subclass for handle type GROUP */
G_DEFINE_TYPE_WITH_CODE (ExampleContactGroup, test_contact_group,
    TEST_TYPE_CONTACT_LIST_BASE,
...
struct _TestContactListBasePrivate
{
...
struct _TestContactListPrivate
{
  int dummy:1;
};
...
static void
test_contact_list_base_init (TestContactListBase *self)
{
  self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
      TEST_TYPE_CONTACT_LIST_BASE, TestContactListBasePrivate);
}

static void
test_contact_list_init (TestContactList *self)
{
  self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, TEST_TYPE_CONTACT_LIST,
      TestContactListPrivate);
}

static void
test_contact_group_init (ExampleContactGroup *self)
{
  self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, TEST_TYPE_CONTACT_GROUP,
...
{
  TestContactListBase *self = TEST_CONTACT_LIST_BASE (object);
  void (*chain_up) (GObject *) =
    ((GObjectClass *) test_contact_list_base_parent_class)->constructed;
  ...
  g_assert (TEST_IS_CONTACT_LIST_MANAGER (self->priv->manager));
  ...
  tp_group_mixin_init (object, G_STRUCT_OFFSET (TestContactListBase, group),
...
{
  TestContactList *self = TEST_CONTACT_LIST (object);
  void (*chain_up) (GObject *) =
    ((GObjectClass *) test_contact_list_parent_class)->constructed;
...
{
  ExampleContactGroup *self = TEST_CONTACT_GROUP (object);
  void (*chain_up) (GObject *) =
    ((GObjectClass *) test_contact_group_parent_class)->constructed;
...
dispose (GObject *object)
{
  ...
  ((GObjectClass *) test_contact_list_base_parent_class)->dispose (object);
}
...
finalize (GObject *object)
{
  ...
  ((GObjectClass *) test_contact_list_base_parent_class)->finalize (object);
}
                  ...
                  GError **error)
{
  ...
  return test_contact_list_manager_add_to_group (self->priv->manager,
                     ...
                     GError **error)
{
  ...
  return test_contact_list_manager_remove_from_group (self->priv->manager,
      object, self->priv->handle, handle, message, error);
}
                 ...
                 GError **error)
{
  ...
  return test_contact_list_manager_add_to_list (self->priv->manager,
      object, self->priv->handle, handle, message, error);
}
                    ...
                    GError **error)
{
  ...
  return test_contact_list_manager_remove_from_list (self->priv->manager,
      object, self->priv->handle, handle, message, error);
}

static void
test_contact_list_base_class_init (TestContactListBaseClass *klass)
{
  ...
  g_type_class_add_private (klass, sizeof (TestContactListBasePrivate));
  ...
  param_spec = g_param_spec_object ("manager", "TestContactListManager",
      "TestContactListManager object that owns this channel",
  ...
  tp_dbus_properties_mixin_class_init (object_class,
      G_STRUCT_OFFSET (TestContactListBaseClass, dbus_properties_class));
...
static void
test_contact_list_class_init (TestContactListClass *klass)
{
  GObjectClass *object_class = (GObjectClass *) klass;

  g_type_class_add_private (klass, sizeof (TestContactListPrivate));

  object_class->constructed = list_constructed;

  tp_group_mixin_class_init (object_class,
      G_STRUCT_OFFSET (TestContactListBaseClass, group_class),
      list_add_member,
      list_remove_member);
...
static void
test_contact_group_class_init (ExampleContactGroupClass *klass)
{
  GObjectClass *object_class = (GObjectClass *) klass;
...
group_channel_close (TpSvcChannel *iface,
                     DBusGMethodInvocation *context)
{
  ExampleContactGroup *self = TEST_CONTACT_GROUP (iface);
  TestContactListBase *base = TEST_CONTACT_LIST_BASE (iface);
...
channel_get_handle (TpSvcChannel *iface,
                    DBusGMethodInvocation *context)
{
  TestContactListBase *self = TEST_CONTACT_LIST_BASE (iface);
